...
class UserArticle extends Model
{
	...
	{
		return $this->request("SELECT * FROM article A RIGHT JOIN (SELECT A.*, UA.unread, UA.later, UA.favourite FROM {$this->table} UA INNER JOIN article A ON UA.articleid = A.id WHERE UA.id = :id) LA ON LA.id = A.id WHERE favourite = 1 GROUP BY A.id", array(
				'id' => $id
			));
	}
	...
	{
		return $this->request("SELECT * FROM article A RIGHT JOIN (SELECT A.*, UA.unread, UA.later, UA.favourite FROM {$this->table} UA INNER JOIN article A ON UA.articleid = A.id WHERE UA.id = :id) LA ON LA.id = A.id WHERE later = 1 GROUP BY A.id", array(
				'id' => $id
			));
	}
	...
	{
		return $this->request("SELECT * FROM article A RIGHT JOIN (SELECT A.*, UA.unread, UA.later, UA.favourite FROM {$this->table} UA INNER JOIN article A ON UA.articleid = A.id WHERE UA.id = '{$id}') LA ON LA.id = A.id WHERE unread = 1 GROUP BY A.id");
	}
